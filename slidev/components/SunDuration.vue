...
<template>
  <div ref="root" class="absolute inset-0" style="pointer-events: none">
    <div
      class="absolute cursor-move select-none rounded flex items-center justify-center"
      :style="{
        left:   pos.x + '%',
        top:    pos.y + '%',
        width:  width + '%',
    ...
    >
      <span class="text-xs text-yellow-900 font-medium">{{ label }}</span>
      <span v-if="dragging" class="absolute bottom-0.5 right-1 font-mono text-[9px] text-yellow-800 opacity-70">
        {{ pos.x }},{{ pos.y }}
      </span>
    </div>
  </div>
