...
// Repeat tick lands at gx(6) exactly (same position as the insertion animation)
const REPEAT_X = SPLIT_X + SPLIT_DX / 2
...
// click layout: 1=Ca dépend, 2=Insertion section, 3–4=insertion anim,
//               5=Smearing section, 6–7=smearing anim, 8=Repeat section appears,
//               9=split, 10=repeat tick
const CLICK_OFFSET = 8

onMounted(async () => {
  await nextTick()

  const rightGroup = document.querySelector('#lsr-right-group')!
  const repeatGroup  = document.querySelector('#lsr-repeat-group')!
  ...
  tl.to(repeatGroup, { opacity: 1, duration: 0.4 })
  tl.addLabel('end-2')
...
<template>
  <div class="w-full py-4">
    <svg :viewBox="viewBox" class="w-full" style="overflow: visible">
      ...
      <!-- Left ticks: 23:59:54 … 23:59:59 (last one in blue — it's the duplicated tick) -->
      <g v-for="(label, i) in leftTicks" :key="label + '-left'">
        <circle
          :cx="gx(i)" :cy="LINE_Y" :r="TICK_RADIUS"
          fill="#fff" :stroke="i === leftTicks.length - 1 ? '#4a9eff' : '#555'" stroke-width="3"
        />
        <text
          :x="gx(i)" :y="LINE_Y + 20"
          text-anchor="middle" dominant-baseline="hanging"
          :fill="i === leftTicks.length - 1 ? '#4a9eff' : '#ccc'" class="tick-label"
        >{{ label }}</text>
      </g>

      <!-- Repeat tick group: hidden until click 2 -->
      <g id="lsr-repeat-group" opacity="0">
        <line :x1="SPLIT_X" :y1="LINE_Y" :x2="REPEAT_X" :y2="LINE_Y" stroke="#ff6b35" stroke-width="3" />
        <line :x1="REPEAT_X" :y1="LINE_Y" :x2="SPLIT_X + SPLIT_DX" :y2="LINE_Y" stroke="#ff6b35" stroke-width="3" />
        <circle :cx="REPEAT_X" :cy="LINE_Y" :r="TICK_RADIUS" fill="#fff" stroke="#ff6b35" stroke-width="3" />
        <text
          :x="REPEAT_X" :y="LINE_Y + 20"
          text-anchor="middle" dominant-baseline="hanging"
          fill="#ff6b35" class="tick-label"
        >{{ repeatTick }}</text>
        <line
          :x1="REPEAT_X" :y1="LINE_Y - TICK_RADIUS - 5"
          :x2="REPEAT_X" :y2="LINE_Y - 30"
          stroke="#ff6b35" stroke-width="2"
        />
        <text
          :x="REPEAT_X" :y="LINE_Y - 34"
